...
This all might sound like cheating, but WoW's UI has always allowed "add-ons" to modify it.  At times, when people have created WeakAuras that were deemed [too powerful](), the WoW development team has reigned the add-on back in by restricting code access to the problematic APIs. 
