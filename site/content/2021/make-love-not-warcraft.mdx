...
If you come from React, this pattern should be immediately recognizable to you. These functions are pretty similar to parts of the old React component lifecycle. I think it's pretty clear to see how this can be powerful, but this example isn't particularly useful. Perhaps, on a specific boss, you want to rotate some of your players to do something (cast a powerful healing spell or something) to save the group every time the boss attacks you with a certain ability. WeakAuras makes that easy. Below, I've adapted the last example to do that.
...
function(event, subevent, spellId)
  if event == "COMBAT_LOG_EVENT" 
    ...
    and spellId === 123456 
  then
    -- #aura_env.rotation is roughly equivalent to 
    -- aura_env.rotation.length in JS
