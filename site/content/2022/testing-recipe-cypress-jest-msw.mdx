...
description: A brief guide on how to share MSW between Jest and Cypress tests
featuredImage: ../../src/assets/images/code__laptop.jpg
featuredIcon: react
author: Chris Carrick
published: 2022-01-13T11:48:12.000Z
tags: ['react', 'cypress', 'jest', 'msw', 'testing', 'javascript', 'typescript']
---
...
test('renders the list of dogs', async () => {
  ...
  const dogs = await screen.findByRole('listitem')
  const milo = await screen.findByRole(
    'listitem', 
    { name: /milo/i }
  )
  const summer = await screen.findByRole(
    'listitem', 
    { name: /summer/i }
  )

  expect(dogs).toHaveLength(2)
  expect(milo).toBeInTheDocument()
